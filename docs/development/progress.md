...
### 🔖 카탈로그 UX 개선 (결정 대기 — 아래 중 선택)

| # | 기능 | 설명 | 난이도 |
|---|------|------|--------|
| ① | **빠른 보기 (Quick View)** | 상품 카드 호버 → 모달로 이미지·색상·사이즈·문의 버튼 확인. 페이지 이동 없이 훑어볼 때 편리 | 중 |
| ② | **상품 비교** | 2~3개 상품 체크 → 사양 나란히 비교 (소재·사이즈·인증·색상). B2B 발주 결정에 유용 | 중 |
| ③ | **최근 본 상품** | 브라우징 중 봤던 상품 하단 플로팅 바 또는 마이페이지 섹션으로 복귀 편의성 | 하 |
| ④ | **리스트 뷰 전환** | 그리드 ↔ 리스트 전환. 리스트 뷰에서 상품명·브랜드·사이즈·색상 한 줄에 표시 | 하 |
| ⑤ | **카탈로그 공유 / 인쇄** | 상품 링크 복사, 카카오 공유, 인쇄 전용 레이아웃. 담당자가 팀에 공유할 때 유용 | 하 |

> 내일 위 목록 보고 하나 이상 선택하면 바로 개발 시작.

### Cloudflare Pages 마이그레이션 (나중에)
...
- **관리자 자수 시안 승인/거절** 기능
- **관리자 쿠폰 생성** 기능
- **`public/products/` 이미지 → Supabase Storage 업로드** 검토
